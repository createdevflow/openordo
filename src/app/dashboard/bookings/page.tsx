...
import { hasFeature, getBookingUsage } from "@/lib/features"
...
export default async function BookingsPage() {
  ...
  const [bookings, doctors, bookingUsage] = await Promise.all([
    db.bookingRequest.findMany({
      where: { clinicId },
      orderBy: { createdAt: "desc" }
    }),
    db.doctor.findMany({ where: { clinicId }, orderBy: { name: "asc" } }),
    getBookingUsage(clinicId)
  ])

  return <BookingsClient bookings={bookings} doctors={doctors} bookingUsage={bookingUsage} />
}
