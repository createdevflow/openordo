...
export async function GET(req: Request) {
  try {
    ...
    // Process documents in background so we can return the stream immediately
    ;(async () => {
      for (const doc of documents) {
        try {
          ...
          if (!safeFileName.includes(".")) {
            // Check URL for extension, or default to .pdf for known types
    ...
    return new NextResponse(readableStream as any, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${clinic.slug || "clinic"}_documents.zip"`
      }
    })
  } catch (err: any) {
